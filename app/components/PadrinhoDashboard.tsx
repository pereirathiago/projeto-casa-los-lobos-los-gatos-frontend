...
export default function PadrinhoDashboard({ user }: PadrinhoDashboardProps) {
  ...
  const handleWhatsAppContact = (animalName: string) => {
    ...
    const userName = user?.name || 'Padrinho';
    const message = encodeURIComponent(
      `Olá! Vim pelo site da Casa Los Lobos e Los Gatos. Meu nome é ${userName} e gostaria de obter mais informações sobre o(a) ${animalName}.`,
    );
    window.open(`https://wa.me/${phone}?text=${message}`, '_blank');
  };

  const handleWhatsAppPhotos = (animalName: string) => {
    const phone = '5542988331566';
    const userName = user?.name || 'Padrinho';
    const message = encodeURIComponent(
      `Olá! Vim pelo site da Casa Los Lobos e Los Gatos. Meu nome é ${userName} e gostaria de receber fotos atualizadas do(a) ${animalName}.`,
    );
    window.open(`https://wa.me/${phone}?text=${message}`, '_blank');
  };
